...
document.querySelectorAll('input[name="action"]').forEach(function (radio) {
    radio.addEventListener('change', function () {
        ...
        if (this.value === 'buy') {
            ...
            sellAmountInput.value = '0'; // Clear the 'amount to sell when switching to sell action
        }
    });
});
...
confirm.addEventListener('click', function () {
    ...
    if (document.querySelector('input[name="action"]:checked').value === 'buy') {
        if (selectedCrypto) {
            ...
            updateWalletText(); // Update the wallet text based on the  purchases for each coin
...
function updateWalletText() {
    ...
    if (walletDetails.length > 0) {
        walletDetails.forEach(crypto => {
            ...
            const profitLoss = crypto.amount * (crypto.sellingPrice - crypto.buyingPrice);
            const profitLossClass = profitLoss >= 0 ? 'profit' : 'loss';
            row.innerHTML = `
            <td>${crypto.symbol}</td>
            <td>${crypto.amount}</td>
            <td>$${crypto.buyingPrice.toFixed(2)}</td>
            <td>$${(crypto.sellingPrice * Math.floor(crypto.amount)).toFixed(2)}</td>
